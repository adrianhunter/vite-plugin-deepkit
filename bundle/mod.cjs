...
var import_ts_clone_node = require("@marcj/ts-clone-node");
var {
  isArrowFunction,
...
function getIdentifierName(node) {
  return import_typescript.default.unescapeLeadingUnderscores(node.escapedText);
}
...
var NodeConverter = class {
  ...
  toExpression(node) {
    ...
    try {
      return (0, import_ts_clone_node.cloneNode)(node, {
...
// type-compiler/src/compiler.ts
var import_vfs = require("@typescript/vfs");
var import_micromatch = __toESM(require("micromatch"), 1);
var import_core = require("@deepkit/core");
var contains = import_micromatch.default.contains;
var {
  visitEachChild,
  visitNode,
  isArrayTypeNode,
...
var ReflectionTransformer = class {
  ...
  transformSourceFile(sourceFile) {
    ...
    if (basePath) {
      basePath = (0, import_node_path.dirname)(basePath);
      ...
      while ((this.reflectionMode === void 0 || this.compilerOptions === void 0) && "string" === typeof basePath && currentConfig.extends) {
        ...
        basePath = (0, import_node_path.dirname)(path);
      }
    }
  ...
  parseReflectionOptionsDefaults(options) {
    options = (0, import_core.isObject)(options) ? options : {};
    if (!options.exclude)
  ...
  readJson(path) {
    try {
      let content = (0, import_node_fs.readFileSync)(path, "utf8");
      content = (0, import_strip_json_comments.default)(content);
  ...
  findReflectionFromPath(path) {
    ...
    let currentDir = (0, import_node_path.dirname)(path);
    let reflection;
    while (currentDir) {
      const packageJsonPath = (0, import_node_path.join)(currentDir, "package.json");
      const tsConfigPath = (0, import_node_path.join)(currentDir, "tsconfig.json");
      let packageJson = {};
      let tsConfig = {};
      ...
      if (packageJsonCache) {
        ...
        packageJsonExists = (0, import_node_fs.existsSync)(packageJsonPath);
        this.resolvedPackageJson[packageJsonPath] = { exists: packageJsonExists, data: {} };
        if (packageJsonExists) {
          try {
            let content = (0, import_node_fs.readFileSync)(packageJsonPath, "utf8");
            content = (0, import_strip_json_comments.default)(content);
      ...
      if (tsConfigCache) {
        ...
        tsConfigExists = (0, import_node_fs.existsSync)(tsConfigPath);
        ...
        if (tsConfigExists) {
          try {
            ...
            while (tsConfig.extends) {
              const file = (0, import_node_path.isAbsolute)(tsConfig.extends) ? tsConfig.extends : (0, import_node_path.join)(dir, tsConfig.extends);
              const ext = this.readJson(file) || {};
              delete tsConfig.extends;
              tsConfig = Object.assign(ext, tsConfig);
              dir = (0, import_node_path.dirname)(file);
            }
      ...
      const next = (0, import_node_path.join)(currentDir, "..");
      if ((0, import_node_path.resolve)(next) === (0, import_node_path.resolve)(currentDir))
        break;
      currentDir = next;
    }
